...
从120组历史偏好样本中，按原始顺序选取前3组中文对话的6条回复，由当前对话助手进行AI辅助评审，记录逐维分数、依据和改进建议，得到4条候选Bad Case。评审已看到模型身份和公开偏好，属于非盲评；API调用0次，人工复核0条。48条自建用例与这6条历史回复是不同数据集，自建用例尚无API实跑结果。评分JSON是已保存的评审记录，不是自动生成的标准答案。

执行 `python scripts/build_assisted_review.py` 可将保存的评审记录与原始回复重新合并，导出中文CSV、统计JSON和简要结果；分数依据见 `downloads/runs/llm-assisted-review.json`。此步骤复算与导出，不会重新执行AI评审。页面反馈流程版本为2026-09-20，源快照为2026-08-14，最近处理时间以 `signal-summary.json` 为准。
